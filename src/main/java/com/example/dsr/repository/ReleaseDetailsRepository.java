...
public interface ReleaseDetailsRepository extends JpaRepository<ReleaseDetails, Long>{
	...
	@Query(value = "select * from release_details where projectid=:project_id and isdelete=0 and isactive=1 order by createddate desc,id desc limit 5",nativeQuery = true)
	List<ReleaseDetails> getlatestVersion(Long project_id);
